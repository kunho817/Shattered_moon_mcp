...
export class AILearningEngine extends EventEmitter {
    ...
    taskMatchesPattern(task, pattern) {
        ...
        if (task.keywords && pattern.keywords) {
            const overlap = task.keywords.filter((k) => pattern.keywords.includes(k)).length;
    ...
    updateRecommendations(type, data) {
    ...
    getRecommendations(context) {
        ...
        frequentErrors.forEach(error => {
            allRecommendations.push(`Frequent error pattern detected: ${error.pattern.tool} - ${error.pattern.errorType}`);
        });
        return [...new Set(allRecommendations)]; // Remove duplicates
    }
    predictTaskOutcome(task) {
