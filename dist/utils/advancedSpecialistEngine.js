...
class AdvancedSpecialistEngine {
    ...
    async createDynamicSpecialist(request) {
        ...
        try {
            ...
            // 7. 전문가 객체 생성
            const specialist = await this.assembleSpecialist(baseType, customization, skillProfile, performanceProfile, adaptationProfile, emergencyCapabilities, request);
            // 8. 진화 계획 수립
            ...
            logger_js_1.default.info('Dynamic specialist created successfully', {
                creationId,
            ...
            logger_js_1.default.error('Dynamic specialist creation failed', {
                creationId,
     ...
     * 기본 전문가 유형 선택
     */
    async selectBaseSpecialistType(request) {
        const prompt = `
...
${Object.keys(index_js_1.SPECIALISTS).map(type => `- ${type}: ${index_js_1.SPECIALISTS[type].description}`).join('\n')}
...
`;
        try {
            ...
            if (selectedType in index_js_1.SPECIALISTS) {
                ...
                logger_js_1.default.warn('AI selected invalid specialist type, using fallback', { selectedType });
                return this.selectFallbackSpecialistType(request);
            }
        }
        catch (error) {
            logger_js_1.default.warn('AI specialist type selection failed, using fallback', { error });
            return this.selectFallbackSpecialistType(request);
        }
    }
    /**
     * 전문가 커스터마이징 생성
     */
    async generateSpecialistCustomization(request, baseType) {
        const prompt = `
...
**Base Capabilities**: ${index_js_1.SPECIALISTS[baseType]?.capabilities.join(', ') || 'Unknown'}
...
`;
     ...
     * 스킬 프로파일 생성
     */
    async generateSkillProfile(request, baseType, customization) {
        const skillProfile = {};
        // 기본 스킬 추가
        const baseSpecialist = index_js_1.SPECIALISTS[baseType];
        if (baseSpecialist?.skills) {
            Object.entries(baseSpecialist.skills).forEach(([skill, level]) => {
                skillProfile[skill] = this.createSkillFromBase(skill, level, customization);
            });
        }
        // 요구되는 스킬 추가/강화
        ...
        if (customization.specializations) {
            customization.specializations.forEach((specialization) => {
     ...
     * 성능 프로파일 생성
     */
    async generatePerformanceProfile(request, skillProfile) {
     ...
     * 적응 프로파일 생성
     */
    async generateAdaptationProfile(request, customization) {
        ...
        for (let i = 0; i < Math.min(10, Math.floor(experienceMonths / 6)); i++) {
            learningHistory.push({
                ...
                learningMethod: ['project', 'training', 'mentorship', 'self_study'][Math.floor(Math.random() * 4)],
     ...
     * 응급 상황 대응 능력 생성
     */
    async generateEmergencyCapabilities(request, baseType) {
        const capabilities = [];
        // 기본 응급 대응 능력
        capabilities.push({
     ...
     * 전문가 객체 조립
     */
    async assembleSpecialist(baseType, customization, skillProfile, performanceProfile, adaptationProfile, emergencyCapabilities, request) {
        ...
        return {
            ...
            skills: skillProfile,
            expertise: [...(index_js_1.SPECIALISTS[baseType]?.capabilities || []), ...customization.specializations],
     ...
     * 진화 계획 생성
     */
    async createEvolutionPlan(specialist, request) {
        const prompt = `
...
`;
        try {
            ...
            logger_js_1.default.warn('AI evolution plan generation failed, using fallback', { error });
     ...
     * 전문가 등록 및 모니터링 설정
     */
    async registerSpecialist(specialist, evolutionPlan) {
        ...
        logger_js_1.default.info('Specialist registered successfully', {
            specialistId: specialist.id,
            type: specialist.type,
            level: specialist.level,
            skillCount: Object.keys(specialist.skills).length
     ...
     * 최적 전문가 매칭
     */
    async findBestSpecialistMatch(request, availableSpecialists) {
        ...
        // 매칭 점수 계산
        const matchResults = await Promise.all(candidates.map(async (specialist) => {
        ...
        logger_js_1.default.info('Best specialist match found', {
     ...
     * 전문가 성능 업데이트
     */
    async updateSpecialistPerformance(specialistId, projectRecord) {
     ...
     * 유틸리티 메서드들
     */
    selectFallbackSpecialistType(request) {
        // 스킬 기반 기본 선택
        const specialistTypes = Object.keys(index_js_1.SPECIALISTS);
        for (const type of specialistTypes) {
            const specialist = index_js_1.SPECIALISTS[type];
    ...
    generateFallbackCustomization(request, baseType) {
        return {
    ...
    createSkillFromBase(skill, baseLevel, customization) {
    ...
    createNewSkill(skill, experienceLevel) {
    ...
    experienceLevelToMultiplier(level) {
        const multiplierMap = {
            'junior': 0.6,
            'mid': 0.8,
            'senior': 1.0,
        ...
        return multiplierMap[level] || 1.0;
    }
    experienceLevelToSkillLevel(level) {
        const skillMap = {
            'junior': 0.3,
            'mid': 0.5,
            'senior': 0.7,
        ...
        return skillMap[level] || 0.5;
    }
    identifyStrengths(skillProfile, performance) {
        const strengths = [];
        // 높은 스킬 레벨 영역
        ...
        // 성능 기반 강점
        if (performance.qualityScore > 0.8)
            strengths.push('high_quality_delivery');
        if (performance.speedScore > 0.8)
            strengths.push('fast_execution');
        if (performance.innovationScore > 0.8)
            strengths.push('innovation');
        if (performance.reliabilityScore > 0.8)
            strengths.push('reliability');
        return strengths.slice(0, 5); // 최대 5개
    }
    identifyWeaknesses(skillProfile, performance) {
        const weaknesses = [];
        // 낮은 성능 영역
        if (performance.speedScore < 0.6)
            weaknesses.push('execution_speed');
        if (performance.innovationScore < 0.5)
            weaknesses.push('innovation');
        if (performance.mentorshipScore < 0.4)
            weaknesses.push('mentorship');
        return weaknesses;
    }
    identifyImprovementAreas(skillProfile, request) {
        const areas = [];
        // 요구 스킬 중 부족한 영역
        request.requiredSkills.forEach(skill => {
    ...
    async calculateMatchScore(specialist, request) {
        let score = 0;
        // 스킬 매칭 (40%)
    ...
    calculateSkillMatch(specialist, request) {
    ...
    calculateExperienceMatch(specialist, request) {
    ...
    identifyMatchStrengths(specialist, request) {
        const strengths = [];
        request.requiredSkills.forEach(skill => {
    ...
    identifyMatchWeaknesses(specialist, request) {
        const weaknesses = [];
        request.requiredSkills.forEach(skill => {
    ...
    async suggestTemporaryEnhancements(specialist, request) {
        const enhancements = [];
        // 스킬 부족 영역에 대한 임시 강화
        request.requiredSkills.forEach(skill => {
    ...
    assessMatchRisks(specialist, request) {
        const risks = [];
        ...
        const skillGap = request.requiredSkills.filter(skill => !specialist.skills[skill] || specialist.skills[skill].level < 0.5).length;
        if (skillGap > 0) {
            risks.push(`${skillGap} required skills below proficiency level`);
        }
        return risks;
    }
    predictPerformance(specialist, request) {
    ...
    calculateMatchConfidence(specialist, request, matchScore) {
        let confidence = matchScore;
        // 과거 성과에 따른 신뢰도 조정
        if (specialist.metadata.successRate > 0.8)
            confidence += 0.1;
        if (specialist.metadata.successRate < 0.6)
            confidence -= 0.1;
        // 경험 기반 조정
    ...
    async recalculatePerformance(specialist, history) {
        if (history.length === 0)
            return specialist.performance;
    ...
    async checkEvolutionTrigger(specialist) {
    ...
    async triggerEvolution(specialist, plan) {
        ...
        if (currentIndex < levelMap.length - 1) {
            specialist.level = levelMap[currentIndex + 1];
            ...
            logger_js_1.default.info('Specialist evolved', {
                specialistId: specialist.id,
    ...
    generateFallbackEvolutionPlan(specialist) {
        return {
            specialistId: specialist.id,
            currentLevel: specialist.level,
    ...
    reset() {
        ...
        logger_js_1.default.info('Advanced specialist engine reset');
    }
}
exports.advancedSpecialistEngine = new AdvancedSpecialistEngine();
//# sourceMappingURL=advancedSpecialistEngine.js.map