...
exports.intelligentTeamMatcher = void 0;
...
class IntelligentTeamMatcher {
     ...
     */
    async findOptimalTeamComposition(task, constraints, preferences, strategy) {
        ...
        logger_js_1.default.info('Starting intelligent team matching', {
            matchingId,
            taskId: task.id,
        ...
        try {
            ...
            logger_js_1.default.info('Candidate pool generated', {
                matchingId,
                candidatesFound: candidatePool.length,
                skillCoverage: this.calculateSkillCoverage(candidatePool, task.requiredSkills)
            });
            // 2. AI 기반 최적 조합 분석
            const optimalCompositions = await this.generateOptimalCompositions(task, candidatePool, constraints, preferences, activeStrategy);
            // 3. 협업 다이내믹스 분석
            const enhancedCompositions = await this.analyzeCollaborationDynamics(optimalCompositions, task);
            // 4. 최종 추천 선택
            const finalComposition = await this.selectFinalComposition(enhancedCompositions, task, constraints, preferences);
            // 5. 성능 예측 및 대안 생성
            await this.generateAlternatives(finalComposition, task, candidatePool, constraints);
            logger_js_1.default.info('Team matching completed successfully', {
                matchingId,
            ...
            logger_js_1.default.error('Team matching failed', {
                matchingId,
     ...
     * 후보 멤버 풀 생성
     */
    async generateCandidatePool(task, constraints, preferences) {
        ...
        let candidates = allMembers.filter(member => {
            ...
            // 스킬 매칭 (최소 하나 이상)
            const hasRequiredSkill = task.requiredSkills.some(skill => member.skills[skill] && member.skills[skill] > 0.3);
            return hasRequiredSkill;
        });
        // AI 기반 후보 확장
        const expandedCandidates = await this.expandCandidatePoolWithAI(candidates, task, constraints, preferences);
        // 스킬 커버리지 확보
        const finalCandidates = await this.ensureSkillCoverage(expandedCandidates, task.requiredSkills);
        return finalCandidates.slice(0, Math.min(50, finalCandidates.length)); // 최대 50명으로 제한
    }
    /**
     * AI 기반 최적 조합 생성
     */
    async generateOptimalCompositions(task, candidatePool, constraints, preferences, strategy) {
        const prompt = `
...
${candidatePool.slice(0, 20).map(member => `${member.name} (${member.team}): Skills[${Object.entries(member.skills).filter(([k, v]) => v > 0.5).map(([k, v]) => `${k}:${Math.round(v * 100)}%`).join(',')}], Exp: ${member.experience}y, Avail: ${Math.round(member.availability * 100)}%`).join('\n')}
...
`;
        try {
            ...
            logger_js_1.default.warn('AI composition generation failed, using fallback algorithm', { error });
     ...
     * 협업 다이내믹스 분석
     */
    async analyzeCollaborationDynamics(compositions, task) {
        const enhancedCompositions = await Promise.all(compositions.map(async (composition) => {
     ...
     * 협업 다이내믹스 계산
     */
    async calculateCollaborationDynamics(members, task) {
        // 팀 응집력 계산
     ...
     * 팀 응집력 계산
     */
    async calculateTeamCohesion(members) {
        let totalCohesion = 0;
        let pairCount = 0;
     ...
     * 의사소통 효율성 계산
     */
    async calculateCommunicationEfficiency(members, task) {
        // 언어 호환성
        ...
        const baseEfficiency = (languageCompatibility * 0.3 +
     ...
     * 갈등 가능성 계산
     */
    async calculateConflictPotential(members) {
     ...
     * 지식 공유 점수 계산
     */
    async calculateKnowledgeSharing(members) {
     ...
     * 의사결정 속도 계산
     */
    async calculateDecisionMakingSpeed(members, task) {
        // 팀 크기 (작을수록 빠름)
     ...
     * 최종 구성 선택
     */
    async selectFinalComposition(compositions, task, constraints, preferences) {
        ...
        logger_js_1.default.info('Final composition selected', {
     ...
     * 구성 점수 계산
     */
    calculateCompositionScore(composition, task, constraints, preferences) {
        const weights = {
            performance: 0.3,
            collaboration: 0.2,
            skillCoverage: 0.2,
        ...
        // 성능 점수
        const performanceScore = (composition.estimatedPerformance.completionProbability * 0.4 +
            composition.estimatedPerformance.qualityExpectation * 0.3 +
            composition.estimatedPerformance.timelineAdherence * 0.3);
        // 협업 점수
        const collaborationScore = (composition.collaborationDynamics.teamCohesion * 0.3 +
        ...
        return (performanceScore * weights.performance +
    ...
    calculateMemberSimilarity(member1, member2) {
        // 스킬 유사성
    ...
    calculateLanguageCompatibility(members) {
        if (members.length === 0)
            return 1;
    ...
    calculateWorkStyleDifference(member1, member2) {
    ...
    calculateSkillDiversity(members) {
        ...
        const totalSkillInstances = members.reduce((sum, member) => sum + Object.values(member.skills).filter(level => level > 0.3).length, 0);
        return totalSkillInstances > 0 ? allSkills.size / totalSkillInstances : 0;
    }
    calculateExperienceBalance(members) {
    ...
    calculateMentoringPotential(members) {
    ...
    calculateLeadershipClarity(members) {
    ...
    calculatePreferencesAlignment(composition, preferences) {
     ...
     * 폴백 및 대안 생성 메서드들
     */
    async generateFallbackCompositions(task, candidatePool, constraints, strategy) {
        // 기본 알고리즘 기반 팀 구성
    ...
    async loadTeamMembers() {
        ...
        const sampleMembers = [];
        Object.entries(index_js_1.VIRTUAL_TEAMS).forEach(([teamName, teamData]) => {
            // 팀당 3-5명의 멤버 생성
            for (let i = 0; i < 4; i++) {
                const member = {
                    ...
                    preferences: {
                        ...
                        communicationFrequency: ['low', 'medium', 'high'][Math.floor(Math.random() * 3)],
                        taskComplexityPreference: ['simple', 'moderate', 'complex'][Math.floor(Math.random() * 3)]
                    },
                    currentProjects: [],
                    timezone: 'UTC+9',
    ...
    generateSkillsForTeam(teamName) {
        const baseSkills = {};
        // 팀별 기본 스킬 할당
        switch (teamName) {
            case 'Backend':
    ...
    async convertAICompositionToTeamComposition(aiComp, candidatePool, task) {
        const members = aiComp.members
            .map((aiMember) => candidatePool.find(c => c.id === aiMember.id || c.name === aiMember.name))
    ...
    async expandCandidatePoolWithAI(candidates, task, constraints, preferences) {
        // AI가 추천할 수 있는 추가 후보들을 찾는 로직
        // 현재는 기존 후보들을 그대로 반환
        return candidates;
    }
    async ensureSkillCoverage(candidates, requiredSkills) {
        // 필수 스킬이 모두 커버되도록 후보 조정
        const coverage = this.calculateSkillCoverage(candidates, requiredSkills);
        if (coverage < 0.8) {
            logger_js_1.default.warn('Insufficient skill coverage in candidate pool', {
                coverage,
                requiredSkills,
    ...
    async generateSmallerTeamAlternative(mainComposition, candidatePool, task) {
        if (mainComposition.members.length <= 3)
            return null;
    ...
    async generateLargerTeamAlternative(mainComposition, candidatePool, task, constraints) {
        if (mainComposition.members.length >= constraints.maxTeamSize)
            return null;
    ...
    async generateSpecializedTeamAlternative(mainComposition, candidatePool, task) {
        // 특정 스킬에 특화된 팀 구성
    ...
    clearCache() {
        ...
        logger_js_1.default.info('Team matching cache cleared');
    }
}
exports.intelligentTeamMatcher = new IntelligentTeamMatcher();
//# sourceMappingURL=intelligentTeamMatcher.js.map