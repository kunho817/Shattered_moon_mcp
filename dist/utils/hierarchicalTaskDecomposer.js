...
exports.hierarchicalTaskDecomposer = void 0;
...
class HierarchicalTaskDecomposer {
     ...
     */
    async decomposeTask(task, context, strategy, decompositionContext) {
        const startTime = Date.now();
        const decompositionId = `hier_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        logger_js_1.default.info('Starting hierarchical task decomposition', {
            decompositionId,
        ...
        try {
            ...
            const strategicTasks = await this.createStrategicTasks(task, context, strategy, decompositionContext);
            // Level 2: Tactical Tasks (15-60분)
            const tacticalTasks = await this.createTacticalTasks(strategicTasks, strategy, decompositionContext);
            // Level 3: Operational Tasks (5-15분)
            const operationalTasks = await this.createOperationalTasks(tacticalTasks, strategy, decompositionContext);
            // Level 4: Atomic Tasks (1-5분)
            const atomicTasks = await this.createAtomicTasks(operationalTasks, strategy, decompositionContext);
            // 메타데이터 생성
            const metadata = await this.generateMetadata(strategicTasks, tacticalTasks, operationalTasks, atomicTasks, strategy);
            const decomposition = {
                id: decompositionId,
                originalTask: task,
                context,
            ...
            logger_js_1.default.info('Hierarchical decomposition completed successfully', {
                decompositionId,
            ...
            logger_js_1.default.error('Hierarchical decomposition failed', {
                decompositionId,
     ...
     */
    async createStrategicTasks(task, context, strategy, decompositionContext) {
        const prompt = `
...
`;
        ...
        try {
            ...
            logger_js_1.default.warn('Failed to parse strategic tasks, using fallback', { parseError });
     ...
     */
    async createTacticalTasks(strategicTasks, strategy, decompositionContext) {
        const allTacticalTasks = [];
...
`;
            ...
            try {
                ...
                logger_js_1.default.warn('Failed to parse tactical tasks, using fallback', {
     ...
     */
    async createOperationalTasks(tacticalTasks, strategy, decompositionContext) {
        const allOperationalTasks = [];
        // 배치 처리로 성능 최적화
        const batchSize = 3;
...
`;
                try {
                    ...
                    logger_js_1.default.warn('Failed to create operational tasks, using fallback', {
     ...
     */
    async createAtomicTasks(operationalTasks, strategy, decompositionContext) {
        const allAtomicTasks = [];
        // 병렬 처리로 성능 최적화
...
`;
            try {
                ...
                logger_js_1.default.warn('Failed to create atomic tasks, using fallback', {
     ...
     * 메타데이터 생성
     */
    async generateMetadata(strategicTasks, tacticalTasks, operationalTasks, atomicTasks, strategy) {
        const totalDuration = strategicTasks.reduce((sum, task) => sum + task.duration, 0);
        // 병렬화 기회 분석
        const parallelismOpportunities = await this.analyzeParallelismOpportunities(strategicTasks, tacticalTasks, operationalTasks);
        // 리스크 평가
     ...
     * 병렬화 기회 분석
     */
    async analyzeParallelismOpportunities(strategicTasks, tacticalTasks, operationalTasks) {
        const opportunities = [];
        // 독립적인 전략 작업 식별
        const independentStrategic = strategicTasks.filter(task => task.dependencies.length === 0 ||
     ...
     * 리스크 평가
     */
    async assessRisks(strategicTasks, tacticalTasks) {
        const highComplexityTasks = strategicTasks.filter(task => task.complexity === 'high' || task.complexity === 'critical').length;
        const highRiskTactical = tacticalTasks.filter(task => task.riskLevel === 'high').length;
     ...
     * 폴백 메서드들
     */
    createFallbackStrategicTasks(task, context) {
        return [
            {
    ...
    createFallbackTacticalTasks(strategicTask) {
        return [
            {
    ...
    createFallbackOperationalTasks(tacticalTask) {
        return [
            {
    ...
    createFallbackAtomicTasks(operationalTask) {
        return [
            {
     ...
     * 성능 메트릭 업데이트
     */
    updatePerformanceMetrics(startTime, decomposition) {
        ...
        const qualityScore = Math.min(1.0, (decomposition.level1.length * 0.4 +
            decomposition.level2.length * 0.3 +
            decomposition.level3.length * 0.2 +
            decomposition.level4.length * 0.1) / 20);
     ...
     * 적응형 전략 추천
     */
    async recommendStrategy(task, context, constraints) {
        const prompt = `
...
`;
        try {
            ...
            logger_js_1.default.warn('Failed to generate strategy recommendation, using default', { error });
            // 기본 전략 반환
            return {
                strategy: 'hybrid',
                maxLevels: 4,
...
exports.hierarchicalTaskDecomposer = new HierarchicalTaskDecomposer();
//# sourceMappingURL=hierarchicalTaskDecomposer.js.map