...
export async function projectMetadata(params) {
...
function getMetadata(metric, projectState) {
    const result = {
    ...
    if (metric) {
        switch (metric) {
            ...
            case 'teams':
                ...
                const activeTeams = Array.from(teamUtil.entries()).filter((entry) => entry[1] > 0).length;
                result.teams = {
...
function updateMetadata(metric, projectState) {
    const result = {
        timestamp: new Date().toISOString(),
        updated: [],
        insights: [],
...
function analyzeMetadata(metric, projectState) {
    const result = {
        timestamp: new Date().toISOString(),
        analysis: {},
        insights: [],
...
function formatMetadataResult(action, result) {
    switch (action) {
        case 'get':
            if (result.overview) {
...
**Trend Analysis**:
${result.trends.map((trend) => `- ${trend.metric}: ${trend.trend} ${trend.change ? `(${trend.change}%)` : ''}`).join('\n')}`;
        default:
