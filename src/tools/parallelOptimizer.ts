import { ParallelOptimizerParams } from '../types/index.js';
...
**Task Distribution**:
${optimizedSchedule.batches.map((batch: any, index: number) => 
  `**Batch ${index + 1}** (${batch.estimatedTime}ms):
${batch.tasks.map((task: any) => `  - ${task.id} (complexity: ${task.complexity})`).join('\n')}`
).join('\n\n')}

**Critical Path Analysis**:
- **Critical Path Length**: ${criticalPath.length} tasks
- **Critical Path Time**: ${criticalPath.reduce((sum: number, task: any) => sum + (task.estimatedTime || 100), 0)}ms
- **Path**: ${criticalPath.map((task: any) => task.id).join(' → ')}
...
function buildDependencyGraph(tasks: any[]): Map<string, any> {
  const graph = new Map();
...
function findCriticalPath(graph: Map<string, any>): any[] {
...
function optimizeTaskScheduling(tasks: any[], threads: number): any {
  const batches: any[] = [];
...
function simulateParallelExecution(schedule: any, threads: number): any {
  let totalTime = 0;
...
function calculateOptimizationMetrics(tasks: any[], executionResult: any): any {
