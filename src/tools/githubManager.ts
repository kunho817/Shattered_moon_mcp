...
async function handleCommit(data: any): Promise<any> {
...
async function handlePush(data: any): Promise<any> {
  const startTime = Date.now();
  const branch = data.branch || 'main';
  const remote = 'origin';
...
async function handlePull(data: any): Promise<any> {
  const startTime = Date.now();
  const branch = data.branch || 'main';
  const remote = 'origin';
...
async function handlePullRequest(data: any): Promise<any> {
  const startTime = Date.now();
...
async function handleIssue(data: any): Promise<any> {
  const startTime = Date.now();
...
function formatOperationResult(action: string, result: any): string {
  switch (action) {
    case 'commit':
