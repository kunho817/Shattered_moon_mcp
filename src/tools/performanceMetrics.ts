...
**Top Performers**:
${result.topPerformers?.map((tool: any) => `- ${tool.name}: ${tool.avgTime}ms (${tool.executions} runs)`).join('\n') || '- No data available'}`;
...
${result.trends?.map((trend: any) => 
  `- **${trend.metric}**: ${trend.direction} trend (${trend.change}% change)`
).join('\n') || '- No significant trends detected'}

**Patterns Identified**:
${result.patterns?.map((pattern: any) => 
  `- ${pattern.description} (Confidence: ${Math.round(pattern.confidence * 100)}%)`
).join('\n') || '- No patterns identified'}

**Predictions**:
${result.predictions?.map((prediction: any) => 
  `- ${prediction.metric}: ${prediction.prediction} (${Math.round(prediction.confidence * 100)}% confidence)`
...
${result.categories.shortTerm?.map((rec: string) => `- ${rec}`).join('\n') || '- No short-term improvements needed'}

**Long-term Optimizations** (Low Priority):
${result.categories.longTerm?.map((rec: string) => `- ${rec}`).join('\n') || '- No long-term optimizations identified'}

**Preventive Measures**:
${result.categories.preventive?.map((rec: string) => `- ${rec}`).join('\n') || '- No preventive measures required'}
