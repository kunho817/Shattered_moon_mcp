import { CodeGenerateParams } from '../types/index.js';
...
function generateComponent(name: string, namespace: string, dependencies: string[]): string {
...
function generateSystem(name: string, namespace: string, dependencies: string[]): string {
  const includes = dependencies.map(dep => `#include "${dep}.h"`).join('\n');
...
function generateShader(name: string, dependencies: string[]): string {
  return `// ${name} Shader
...
function generateEvent(name: string, namespace: string): string {
  return `#pragma once
...
function generateUtility(name: string, namespace: string, dependencies: string[]): string {
  const includes = dependencies.map(dep => `#include "${dep}.h"`).join('\n');
...
function applyOptimizations(code: string, optimizations: string[]): string {
