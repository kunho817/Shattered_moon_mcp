...
export async function dynamicTeamExpander(params: DynamicTeamExpanderParams) {
...
${conflicts.map((c: any) => 
  `- **${c.specialist}**: ${c.issue}${c.currentLoad ? ` (${c.currentLoad}% loaded)` : ''}
...
${teamOptimization.suggestedAdditions.map((s: string) => `- ${s} (${SPECIALISTS[s as keyof typeof SPECIALISTS]?.expertise || 'Unknown expertise'})`).join('\n')}` : ''}
