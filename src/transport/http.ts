...
import { SecurityValidator, RateLimiter } from '../utils/security.js';
...
export class StreamableHTTPTransport extends EventEmitter {
  ...
  private setupMiddleware(): void {
    ...
    this.app.use((req, res, next) => {
      logger.info(`HTTP ${req.method} ${req.path}`, {
        ...
        headers: SecurityValidator.maskSensitiveData(req.headers)
      });
      next();
    });
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/state/:namespace', (req, res) => {
      const { globalStateManager } = getServices();
  ...
  private handleSSEConnection(req: Request, res: Response): void {
    ...
    logger.info(`SSE client connected: ${clientId} with session: ${sessionId}`);
  ...
  private async handleMCPMessage(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Input validation
      const sanitizedMessage = SecurityValidator.maskSensitiveData(message);
      ...
      if (SecurityValidator.detectInjection(messageStr)) {
        logger.warn('Potential injection detected in MCP message', {
          clientId,
  ...
  private sendSSEMessage(client: SSEClient, message: any): void {
    try {
      const data = JSON.stringify(message);
      client.response.write(`data: ${data}\n\n`);
    } catch (error) {
      logger.error(`Failed to send SSE message to ${client.id}`, { error });
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.server.on('error', (error: any) => {
          logger.error('HTTP Transport start error', { error });
          reject(error);
        });
      } catch (error) {
        reject(error);
