...
export class ClaudeCodeInvoker {
  private static instance: ClaudeCodeInvoker;
  private requestCount = 0;
  private cache = new Map<string, ClaudeCodeResponse>();
  ...
  classifyTask(description: string, context?: string): TaskClassification {
    ...
    planningKeywords.forEach(keyword => {
      if (text.includes(keyword)) planningScore++;
    });

    executionKeywords.forEach(keyword => {
      if (text.includes(keyword)) executionScore++;
    });
    ...
    const suggestedModel: 'opus' | 'sonnet' = isPlanningTask ? 'opus' : 'sonnet';
   ...
   */
  async invoke(prompt: string, options: ClaudeCodeOptions = {}): Promise<ClaudeCodeResponse> {
    ...
    if (this.cache.has(cacheKey)) {
      ...
      logger.info('Claude Code cache hit', { 
    ...
    try {
      ...
      logger.error('Claude Code invocation failed', {
        requestId: this.requestCount,
        error: result.error,
        duration: result.duration
      });
   ...
   */
  async invokePlanning(prompt: string, options: Omit<ClaudeCodeOptions, 'model'> = {}): Promise<ClaudeCodeResponse> {
   ...
   */
  async invokeExecution(prompt: string, options: Omit<ClaudeCodeOptions, 'model'> = {}): Promise<ClaudeCodeResponse> {
   ...
   */
  async invokeAuto(prompt: string, context?: string, options: Omit<ClaudeCodeOptions, 'model'> = {}): Promise<ClaudeCodeResponse> {
    const classification = this.classifyTask(prompt, context);
    
    logger.info('Auto-classified task', {
  ...
  private buildClaudeArgs(prompt: string, options: ClaudeCodeOptions): string[] {
    const args = ['claude'];
  ...
  private executeClaudeCommand(args: string[], timeout: number): Promise<{ success: boolean; output: string; error?: string }> {
  ...
  private generateCacheKey(prompt: string, options: ClaudeCodeOptions): string {
  ...
  clearCache(): void {
    this.cache.clear();
    logger.info('Claude Code cache cleared');
  }
...
export const claudeCodeInvoker = ClaudeCodeInvoker.getInstance();